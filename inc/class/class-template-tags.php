...
class Inx_Game_Post_Meta {
	...
	public function __construct() {
		...
		add_action( 'inx_game_do_tags', array( $this, 'render_tags_list' ) );
		add_action( 'inx_game_site_content_type', array( $this, 'render_meta_list' ), 20, 1 );
		add_action( 'inx_game_meta_info', array( $this, 'render_meta_list' ), 10, 2 );
	...
	public function render_meta_list( $order, $wrp_class = '' ) {
		...
		foreach ( $order as $meta ) {
			switch ( $meta ) {
				case 'author':
					...
					$markup .= ' <span>'. esc_html__( 'By -','inx-game' ) .'</span>';
				...
				case 'date':
					...
					$markup .= esc_html__( 'Posted on ','inx-game' );
					$markup .= $this->get_time_tags();
					$markup .= '</li>';
					break;
				case 'category':
					$markup .= '<li class="meta category">';
					$markup .= esc_html__( 'Posted in ','inx-game' );
		...
		echo wp_kses( $markup, inx_game_alowed_tags() );
	}
	...
	private function get_comments() {
		...
		if ( $comments_number == 0 ) {
			...
			$comments = sprintf( _n( '%s Comment', '%s Comments', $comments_number, 'inx-game' ), $comments_number );
		}
	...
	public function render_tags_list() {
		...
		$html .= '<span>' . esc_html__( 'Tags', 'inx-game' ) . ':</span>';
		foreach ( $tags as $tag ) {
