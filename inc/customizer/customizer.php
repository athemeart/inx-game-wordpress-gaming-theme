...
function inx_game_customize_register( $wp_customize ) {
	...
	$wp_customize->register_section_type( 'inx_game_Customize_Section_Upsell' );

	// Register sections.
	$wp_customize->add_section(
		new inx_game_Customize_Section_Upsell(
			$wp_customize,
			'theme_upsell',
			array(
				'title'    => esc_html__( 'Upgrade to INXGAME Pro!', 'inx-game' ),
				'pro_text' => esc_html__( 'Go PRO', 'inx-game' ),
				'pro_url'  => 'https://athemeart.com/downloads/inx-game-wordpress-casino-theme/',
				'priority'  => 1,
			)
		)
	);
