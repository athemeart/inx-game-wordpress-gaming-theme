    ...
    public function __construct(array $atts ) {
        ...
        $php_classes = array(
            ...
            'fa-iconpicker' => array(
			 ...
			 // Textarea field
            'checkbox' => array(
                'class' => 'Predic_Widget_Check_Field',
                'path' => PREDIC_WIDGET_ROOT_PATH . '/class/fields/class-predic-widget-check-field.php'
            ),
        );
        ...
        $pairs = array(
            ...
            'widget_ops' => array(
                ...
                'description' => esc_html__( 'Some description for example widget.', 'inx-game' ),
        ...
        if ( empty( $this->atts['callback'] ) ) {
            trigger_error( esc_html__( 'Please provide setup param: callback.', 'inx-game' ), E_USER_ERROR );
            return false;
        }
        
        if ( empty( $this->atts['base_id'] ) || empty( $this->atts['name'] ) ) {
            trigger_error( esc_html__( 'Please provide setup params: id and name.', 'inx-game' ), E_USER_ERROR );
            return false;
        }
    ...
    public function form( $instance ) {
        ...
        foreach ( $this->form_fields as $name => $field ) {
            ...
            if ( is_int( $name ) ) {
                ...
                trigger_error(  sprintf( esc_html__( 'You should not use numbers as widget admin form field id. Id provided is: %d.', 'inx-game' ), intval( $name )  ), E_USER_NOTICE );
            }
            ...
            if ( file_exists( $class_path ) ) {
				...
				/* translators: %s: plugin name. */
                trigger_error(  sprintf( esc_html__( 'File %s does not exists.', 'inx-game' ), esc_attr( $class_path ) ), E_USER_WARNING );
                continue;
            }
            ...
            if ( $form_field instanceof Predic_Widget_Form_Field ) {
				...
				/* translators: %s: plugin name. */
                    esc_attr( sprintf( __( '%s class must extend abstract class Predic_Widget_Form_Field', 'inx-game' ), get_class( $form_field ) ) ), 
                    E_USER_ERROR 
                );
                return false;
            }
    ...
    public function fields_admin_scripts() {
        ...
        foreach ( $this->field_php_classes as $class ) {
            ...
            if ( file_exists( $class['path'] ) ) {
				...
				/* translators: %s: plugin name. */
                trigger_error(  sprintf( esc_html__( 'File %s does not exists.', 'inx-game' ), esc_attr( $class['path'] )  ), E_USER_WARNING );
                continue;
            }
