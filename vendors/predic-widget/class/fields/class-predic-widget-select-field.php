...
class Predic_Widget_Select_Field extends Predic_Widget_Form_Field {
    ...
    public function __construct( $atts, $id, $name, $value ) {
		...
		$this->multiple = isset( $atts['multiple'] ) && !empty( $atts['multiple'] ) ? 'multiple' : ''; // Options
		
	 	$this->css_class = isset( $atts['class'] ) && !empty( $atts['class'] ) ? $atts['class'] : ''; // Label
    ...
    public function field() {
        ...
        // Html output
        $html .= '<select class="widefat '.esc_attr(  $this->css_class ).'" '
                . 'id="' . esc_attr( $this->id ) . '" '
                . 'name="' . esc_attr( $this->name ) . '" '
				.  esc_attr($this->multiple)
                . '>';
