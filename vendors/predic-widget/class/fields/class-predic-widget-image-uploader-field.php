...
class Predic_Widget_Image_Uploader_Field extends Predic_Widget_Form_Field {
    ...
    public function field() {
        ...
        // Select button
        $html .= '<button type="button" class="button predic-widget-img-uploader__select">' . esc_html__( 'Select', 'inx-game' ) . '</button>';
        // Clear button
        $html .= '<button type="button" class="button predic-widget-img-uploader__clear">' . esc_html__( 'Clear', 'inx-game' ) . '</button>';
        
        // Default value
        if ( !empty( $this->default ) ) {
            $html .= '<button type="button" class="button predic-widget-img-uploader__default" data-url="' . esc_url( $this->default ) . '">' . esc_html__( 'Default', 'inx-game' ) . '</button>';
        }
    ...
    public static function admin_scripts( $hook ) {
        ...
        wp_localize_script( 'predic-widget-uploader-field', 'predic_widget_uploader_field', array(
           'uploader_title' => esc_html__( 'Upload or select image', 'inx-game' ),
           'button_text' => esc_html__( 'Select image', 'inx-game' )
        ));
